// Slices are an important data type in So, giving
...
func main() {
	...
	// To create a slice with non-zero length, use the `make` builtin.
	...
	// if we know the slice is going to grow, we must pass a capacity
	// as an additional parameter to `make` (`6` in this case).
	s = make([]string, 3, 6)
	printSlice("emp:", s)
	println("len:", len(s), "cap:", cap(s))
	...
	// The `append` builtin returns a slice containing one or
